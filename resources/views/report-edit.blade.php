...
<body>
    <div class="shell">
        <div class="topbar">
            ...
            <a href="{{ route('student.dashboard') }}" class="ghost-btn">Kembali</a>
        </div>
        ...
        <div class="card">
            ...
            <div class="two-col">
                <label>
                    ...
                    <select name="type" required>
                        <option value="aspirasi" {{ old('type', $report->type) === 'aspirasi' ? 'selected' : '' }}>Aspirasi & saran</option>
                        <option value="pengaduan" {{ old('type', $report->type) === 'pengaduan' ? 'selected' : '' }}>Pengaduan aman</option>
                        <option value="lost_found" {{ old('type', $report->type) === 'lost_found' ? 'selected' : '' }}>Barang hilang & temuan</option>
                    </select>
                </label>
                ...
                <label>
                    ...
                    <select name="category" required>
                        <option value="Fasilitas" {{ old('category', $report->category) === 'Fasilitas' ? 'selected' : '' }}>Fasilitas sekolah</option>
                        <option value="Kurikulum" {{ old('category', $report->category) === 'Kurikulum' ? 'selected' : '' }}>Kurikulum & pembelajaran</option>
                        <option value="Kedisiplinan" {{ old('category', $report->category) === 'Kedisiplinan' ? 'selected' : '' }}>Kedisiplinan & keamanan</option>
            ...
            <label>
                Judul laporan
                <input type="text" name="subject" value="{{ old('subject', $report->subject) }}" required>
            </label>
            ...
            <label>
                ...
                <div class="actions">
                    <label class="checkbox-row">
                        <input type="checkbox" name="is_anonymous" value="1" {{ old('is_anonymous', $report->is_anonymous) ? 'checked' : '' }}>
                        <span>Kirim sebagai anonim</span>
                    </label>

                    <div style="display:flex; gap:12px; flex-wrap:wrap;">
                        <a href="{{ route('student.dashboard') }}" class="ghost-btn">Batal</a>
